...
Researchers employ various experimental methods to study how transcription factors interact with DNA, identifying sequences or genomic regions that can reveal the TF specificity for particular DNA patterns. However, obtaining where all TFs bind in all cell types and conditions is intractable. In addition, assessing how mutations could alter binding is also difficult. To address these issues, researchers rely on computational modeling modeling of TF-DNA interactions. Different models are available today, from simple and widely used \ac{PWMs} to advanced deep learning models. This chapter will cover different models and techniques used to create them, highlighting resources that store these models and explore their applications in research.
  ...
  \section{Transcription factor DNA binding models}
    ...
    The most popular and widely used representations of TF-DNA interactions are the \ac{PWMs} (also called weight matrices or \ac{PSSMs}) ({Figure \ref{tf_dna_models}}) \cite{ambrosini2020insights,weirauch2013evaluation,stormo2015dna}. PWMs represent the binding preferences of TFs to DNA sequences. Several steps are followed when building a PWM.
  ...
  \end{table}
    ...
    \begin{figure}
      ...
      \caption[NMF application in SeqArchR]{\textbf{SeqArchR applies NMF to identify position-specific DNA patterns in promoter regions.} \textbf{A.} Input N number of DNA sequences of length \textit{L} are represented as a one-hot encoded matrix (0/1 matrix). The input sequences should be aligned using a reference point, such as a transcription start site (TSS). The NMF algorithm is applied to factorize the input matrix into the amplitude and pattern matrices, asking for four DNA patterns. \textbf{B.} The discovered patterns are visualized. Adapted from \cite{nikumbh2023identifying}.}
      \label{seqarchr}
    \end{figure}
